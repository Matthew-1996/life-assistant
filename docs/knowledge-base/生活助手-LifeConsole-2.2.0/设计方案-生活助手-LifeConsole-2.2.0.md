...
- 2026-08-13 的后续 PO 决策已覆盖 Gate 2 的 Email OTP 基线：正式入口使用管理员预创建 Owner 的邮箱 + 密码登录，不开放注册。
- Magic Link 仅用于“忘记密码”恢复；恢复请求使用中性反馈，不暴露邮箱是否存在。
- Preview 与 Production 使用各自的 Site URL 和 `/auth/recovery` redirect allowlist，避免跨环境回跳。
- 首版不开放注册；未知邮箱和错误密码使用不泄露账号状态的提示。
...
| PO | 已确认 | 2026-08-12 已确认 O1-O5 |

## 8. Gate 2 设计决策

- O1：历史 Gate 2 基线为 Email OTP；2026-08-13 已由邮箱 + 密码、管理员预创建 Owner、禁用公开注册的后续决策覆盖。
