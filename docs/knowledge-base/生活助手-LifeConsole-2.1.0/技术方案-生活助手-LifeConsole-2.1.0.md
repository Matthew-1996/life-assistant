...
- `SystemPage` 不渲染迁移阶段、迁移卡片或迁移向导；备份卡片增加静态 Agent 恢复说明。
- `ProgressPage` 删除“恢复边界”工程指标；迁移状态不参与生活进展展示。
- `migration_state`、迁移 API 与受控工具契约继续保留，前端隐藏不作为安全控制或删除后端能力的理由。
...
| Q8 | 迁移/恢复退出前端信息架构；内部状态机、`verify` / `restore-plan` 与 PO 门禁继续保留 |
...
- Gate 2C：PO 已确认 Q8 与对应设计 O6-O7，授权更新通用前端、知识库和合成测试；不授权正式部署、真实数据操作、切源、资源删除或 PR 合并。
