...
- 网站不提供迁移/恢复独立模块、迁移向导或迁移阶段展示；低频重建由 Agent 基于 iCloud 备份协助完成。
...
- 2.1.0 不提供「从 iCloud 恢复」在线入口；只保证格式可恢复，并由未来 Agent 受控流程完成重建。
- 备份卡片仅用一句弱提示说明“可在换机或项目重建时由 Agent 协助恢复”，不提供恢复按钮、迁移入口或技术步骤。
...
- 系统页删除“迁移与恢复边界”卡片、迁移向导与迁移阶段；进展页不展示“恢复边界”指标。
...
- 上述能力只保留在内部接口、恢复工具契约与项目知识库中，不作为网站信息架构或日常入口。
...
| D9 | 网站只保留 iCloud 最新备份，不展示迁移/恢复模块 | 恢复由未来 Agent 受控流程完成；内部迁移状态机、校验和门禁继续保留 | 降低日常认知负担，不削弱可恢复性与安全审批 |
...
## 8.3 Gate 2C 网站恢复入口减法（PO 已确认）

- iCloud 最新备份是网站唯一面向用户的备份/恢复相关模块。
- 删除系统页“迁移与恢复边界”卡片、迁移向导、迁移阶段文案，以及进展页“恢复边界”展示。
- 备份卡片增加一句 Agent 恢复用途说明，不增加在线恢复按钮。
- 迁移状态机、格式校验、`verify` / `restore-plan` 契约和高风险操作门禁继续保留在内部能力与项目知识库。
- 本确认允许更新 2.1.0 方案、通用前端代码和合成测试；不包含正式部署、真实数据、切源、资源删除或 PR 合并。
...
- Gate 2C：PO 于 2026-08-12 确认采用“网站只保留 iCloud 最新备份、以后通过 Agent 对话恢复”的新方案，并要求先更新方案评审再改代码。
...
| 2026-08-12 | 2.1.0-draft.7 | Gate 2C：网站移除迁移/恢复模块与阶段展示，iCloud 最新备份成为唯一相关入口，未来恢复交由 Agent 受控执行 | 开发与联调 |
