...
| 编号 | 待确认项 | Agent 推荐 | 影响 |
|---|---|---|---|
| D1 | 云端字段级加密是否保留 | 保留云端 AES/KEK；取消恢复包 | 保持云端数据保护，同时显著简化用户交互 |
| D2 | iCloud 最新备份的数据形态 | 导出可重建的规范化明文 JSON/ZIP，由 iCloud 账户保护 | 取消恢复包后仍能独立重建；需 PO 接受本地备份为可读数据 |
| D3 | 备份触发方式 | 首版手动按钮；定时备份延后 | 最小实现、失败更容易理解 |
| D4 | 浏览器如何写入 iCloud | 复用受控本地同步代理，原子覆盖唯一文件；网页只发起/展示任务 | 浏览器不能直接稳定写入指定 iCloud 路径，需要本地代理在线 |
| D5 | 2.1.0 是否实现在线恢复 UI | 不实现；只保证备份格式与恢复工具契约 | 避免重新引入复杂恢复流程 |
| D6 | 审计数据 | 前端隐藏，后端保留最小事件 | 支持故障排查和高风险删除保护 |
| D7 | R2 处置 | 2.1.0 不再依赖 R2 做恢复包；现有资源在上线后另设清理门禁 | 不在评审期删除资源，避免误删未知对象 |
| D8 | PR #38 处置 | 保持 Draft；Gate 2 后从其提炼有效阶段 C 能力，由 2.1.0 实现 PR 取代 | 避免原样合并失败方案，也避免丢失可复用代码 |
...
- 当前状态：待确认。
- 确认对象：本 PRD draft.1、需求评审报告及 D1-D8。
