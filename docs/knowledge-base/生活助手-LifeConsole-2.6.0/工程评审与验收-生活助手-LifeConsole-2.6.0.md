...
- PO 于 2026-08-31 确认 Preview“已验收，符合预期”，随后单独确认合并与上线。PR [#82](https://github.com/Matthew-1996/life-assistant/pull/82) 三项 CI 全绿后 squash merge，准确 `main` 为 `4274998`。
- Production 从准确 `main` 进行 Vercel 远端构建，构建前环境守卫通过并完成 130 个模块；部署为 READY，稳定正式别名指向新制品。未使用本地 `--prebuilt` 制品。
- 正式域名首页和 Manifest 为 200；CSP 保持 `script-src 'self'`，`nosniff`、`DENY` 保留；`/service-worker.js` 为 404；未登录空 POST `/api/journal-normalize-health` 返回 `401 / no-store`。
- 真实浏览器 390×844 下只读打开一个现有 Todo 的编辑弹窗，没有提交表单。Todo 项目、优先级、计划开始、DDL 四个控件均为 `45–345px`、宽 `300px`，两个日期时间控件右侧溢出为 0；根页面与 body 宽均为 390px，控制台无警告或错误。验收后已关闭弹窗并恢复默认视口。
