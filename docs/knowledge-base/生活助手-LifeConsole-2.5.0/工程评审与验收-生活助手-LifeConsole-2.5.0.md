...
- 增量提交后的 Node CI 连续两次只在同一 NodeNext 类型检查触发默认 5 秒超时；最近成功基线该测试已耗时 4.919 秒，新增 PGlite 文件并行后升至约 5.95 秒，其余 542 项通过。将 Vitest worker 从 3 降为 2、保持原超时后，本地 UTC 完整套件 75 文件 / 543 项通过，该类型检查约 1.0 秒；等待最新远端 CI 复验。
