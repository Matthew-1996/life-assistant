...
当前状态：2.5.0 主功能已发布。PR #74 已按 PO 当次确认合并，并从准确 `main` 发布到 Production；Owner 浏览器确认主观信号两段窗口与睡眠时长趋势恢复，记录页指定待整理日记已通过 revision-safe 原子路径完成结构化回写。活动最近窗口仍缺少 Owner-scoped `health_days` 来源，不伪造或跨源补值；该次真实日记的 DeepSeek 严格契约被拒后按设计失败关闭，改用既有 Agent 整理路径完成，因此“以后新日记自动由 DeepSeek 整理”的首个真实成功样本仍待后续自然新增验证。每日新闻前端展示继续保持搁置，历史定位与恢复门禁见“每日新闻搁置与接力”。
