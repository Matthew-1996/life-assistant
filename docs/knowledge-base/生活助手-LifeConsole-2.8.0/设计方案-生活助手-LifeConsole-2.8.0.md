...
状态：Gate 2 已由 PO 于 2026-08-31 确认；经 Preview 真机参照反馈，当前 62px 高 / 31px CSS 圆角 / 20px 侧距候选已通过技术验收与 PO 产品验收。
...
| 外层导航高度 | 62px |
| 外层圆角 | 31px，与高度保持半径同心 |
| 左右最小间距 | 20px，并叠加横向 safe-area 保护 |
...
在窄屏或横屏中，左右间距使用 `max(20px, env(safe-area-inset-left/right))`，不允许胶囊进入设备不可交互区域。参照合成图中约 50px 的曲率是缩放后像素，换算为约 31px CSS 半径，因此不将 CSS 圆角改为 50px。
...
1. 390×844 合成页面中，导航为 62px 浮动胶囊，左右 20px，内容在其周围可见。
