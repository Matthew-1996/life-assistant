...
状态：Gate 2 已由 PO 于 2026-08-31 确认；本地实现进行中。
...
| 距 Home Indicator 安全区 | `8px + env(safe-area-inset-bottom)` |
| 外层内边距 | 5px |
...
- 底栏背景浮于全屏内容之上，交互元素位于 `safe-area-inset-bottom` 以上。
...
1. 390×844 合成页面中，导航为 64px 浮动胶囊，左右 12px，内容在其周围可见。
2. Home Indicator 区域由页面背景延伸，但交互内容不进入不安全区。
