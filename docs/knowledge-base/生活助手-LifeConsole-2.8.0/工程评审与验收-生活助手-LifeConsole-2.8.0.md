...
状态：已完成。20px 侧距修订已通过 TDD、完整本地回归、PR CI、纯静态 Preview 技术验收、PO 产品验收、PR 合并、Production 远端构建与上线后只读验收。
...
- HTTP 门禁不读取或写入真实日记、健康数据及其他 Owner 内容；浏览器仅加载既有 Owner 页面进行几何与导航只读检查，不复制业务内容、不操作表单，也不触发模型、Cron、迁移或数据库写入。

## 7. Production 上线验收

- PR #84 已 squash 合并，准确 `main` 为 `5150f96dd80c71ec4a20509bd83c2a797f4831fa`。
- Vercel 从准确 `main` 远端执行 Production 构建，状态 READY；稳定域名已指向本次部署，未使用本地 `--prebuilt` 产物。
- 首页与 Manifest 200；Service Worker 404；严格 CSP、`nosniff`、`DENY` 保持；未认证空 POST 健康门禁为 401 / `no-store`。
- 402×874 浏览器实测继续得到 62px 高、31px 圆角、左右 20px、底距 8px及 85×52px 点击区；四页切换、滚动常驻和控制台检查通过。
- 最近一小时 Vercel error 日志为 0；每日新闻既有读取失败保持为 2.5.0 范围边界，本版本未修改或宣称修复。
- 完整去敏证据见[上线证据](上线证据-生活助手-LifeConsole-2.8.0.md)。
