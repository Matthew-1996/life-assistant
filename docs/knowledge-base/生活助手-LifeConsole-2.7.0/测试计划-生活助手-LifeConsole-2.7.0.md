# 测试计划：Life Console 2.7.0

状态：待测试（已完成）。

## 1. 功能矩阵

| 场景 | 预期 |
|---|---|
| “今日”首次加载 | 未开始 / 进行中可见，已完成在列表与甘特图隐藏 |
| 切换“全部” | 保留筛选选择，已完成仍默认隐藏 |
| 选中已完成 | 完成项同时回到列表与甘特图 |
| 进行中改为已完成 | 当已完成未入选时，行与甘特图立即消失 |
| 取消全部三个状态 | 列表和甘特图显示筛选专用空状态 |
| 打开筛选浮层 | 下方新建表单位置不变，连续多选不自动关闭 |
| 点击选项行文字或空白区域 | 每次只切换一次对应状态，浮层保持展开 |
| 关闭筛选浮层 | 再次点击、点击外部或 `Escape` 均可关闭；`Escape` 恢复触发器焦点 |
| 390px 视口 | 触发器紧凑；checkbox 本体为 12–16px，选项行不低于 36px；浮层在 Todo 面板内且不重叠 |

## 2. 自动化

- React 单元 / 交互测试覆盖默认筛选、两个 Tab、回捞、状态流转、空状态、多选保持展开、点击外部与 `Escape` 关闭及焦点恢复。
- Playwright 合成测试在 390×844 下测量紧凑触发器、checkbox 本体、浮层前后表单位置、选项边界、高度和两两交集，并从距选项行右边缘 8px、远离 checkbox 的位置连续点击验证整行命中。
- 复跑完整 `npm test`、生产构建、合成 Playwright、根目录工具测试、治理检查与 Git 隐私检查。

## 3. 验收边界

合成数据与浏览器几何验证不读写 Owner 真实数据。本地、CI、Draft PR 或 Preview 技术验收通过不等于 PO 产品验收，也不授权合并或发布。

## 4. 执行结果

- 视觉细化与键盘焦点定向组件测试 21/21 通过；完整 Vitest 82 个文件、617 项通过。
- 应用内 Python 测试 75 + 7 + 10 + 1 项通过。
- 完整 Playwright 14/14 通过，包含新增 390px 紧凑浮层与无布局位移几何用例。
- checkbox 尺寸断言先在三个状态上全部红灯，再以 14×14px 最小覆盖转绿；选项行仍不低于 36px。
- 整行点击用例先复现“点击远离 checkbox 的空白区域后浮层关闭、checkbox 无法稳定切换”，再将焦点目标未知时的关闭判断延后一拍；同一点连续点击后分别选中、取消，且浮层始终展开。
- Supabase Production 与 Candidate Preview 模式构建通过，130 个模块完成转换；Production 制品不包含候选 Todo 标题。
- Vercel Preview 为 10 个静态文件、0 Functions、0 Cron；`/api/*` 返回 404，CSP 为 `connect-src 'none'`、`script-src 'self'`。
- 替换 Preview 的桌面真实浏览器验收通过：checkbox 14×14px、选项行 38px、触发器 32px，浮层不改变表单文档位置；多选、两个 Tab 保持、点击外部与 `Escape` 关闭均符合预期。
- 替换 Preview 的 390×844 真实浏览器验收通过：checkbox 14×14px、选项行 38px，浮层位于 Todo 面板内，无页面横向溢出，Todo 面板与表单 `offsetTop` 在展开前后不变。
- 最新整行点击替换 Preview 在 390×844 真实浏览器验收通过：点击点距选项行右边缘 8px、距 checkbox 右侧 150px；第一次点击后已完成选中且浮层展开，第二次同点点击后取消且浮层仍展开。
- 根工具测试 372 项通过，1 项既有跳过；治理、Git 隐私与差异检查通过。

沙箱首跑拒绝 Miniflare、Playwright 与根工具的 `127.0.0.1` 回环端口；在允许本机回环监听的同一 worktree 原样重跑后全部通过，未放宽断言或超时。
