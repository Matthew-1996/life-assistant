...
状态：已完成。PO 产品验收、PR #79、PR #80、远端 Production 构建、稳定域名与 Owner 真实浏览器只读验收均通过；首次部署事故已回滚并完成防回归修复。
...
- `python3 tools/validate_project.py` 已执行；其全工作区便携性扫描仍因既有 `.vercel/output` 绝对路径、私人职业资料启发式命中，以及隔离 worktree 不包含 Git 禁止跟踪的私人真相源而返回失败。主工作区复核得到同类既有问题；PR #79 的变更路径未新增这些命中，定向治理、索引隐私与 `origin/main..HEAD` 历史隐私检查均通过。
...
PO 于 2026-08-29 确认最新替换 Preview 产品验收通过，随后明确授权合并与 Production。发布必须使用合并后的准确 `main` 构建并按实际证据回填；当前授权不扩展为数据迁移、API、Supabase、Cron、自动化或其他版本发布。

## 6. Production 发布事故与恢复

- 首次发布使用 `vercel pull --environment=production` 后的本地预构建；Vercel 对敏感环境变量返回字面占位符 `[SENSITIVE]`，该值进入浏览器 Production bundle，导致现有 Owner 会话通过 Auth Gate 后无法读取工作台。
- 事故与 Todo 状态筛选投影、数据库结构、RLS、RPC 和真实数据无关；PR #79 对 Production 认证链路没有代码改动。
- 正式域名已回滚到上一已知可用制品；回滚后同一 Owner 会话无需重新登录即可读取工作台，验证账号、会话和数据库未受损。
- 修复先以失败测试证明占位符此前仍可完成 Production 构建，再增加构建前公钥校验；占位符现在必须在 TypeScript/Vite 编译前失败，合法合成公钥仍可完成 130 模块构建。
- 修复分支应用测试 618/618、应用 Python 75 + 7 + 10 + 1、根目录 372 项（1 项本机私有契约跳过）均通过；其中回环监听用例按运行权限边界单独复验通过。治理、Git 隐私索引与差异检查通过。
- `validate_project.py` 仍报告 worktree 不包含 Git 排除的私有真相源、既有旧链接与测试夹具启发式命中；这些结果在本次差异前已存在，未作为本修复新增通过声明。
- 重新发布必须由 Vercel 远端构建读取 Production 环境，不再使用包含敏感占位符的本地 `--prebuilt` 制品；稳定域名与 Owner 浏览器复验通过前不得标记 2.7.0 上线完成。

最终重新发布满足上述条件：Vercel 远端从提交 `feb5c2e3` 构建，稳定域名首页与安全头通过，既有 Owner 会话恢复；“今日 / 全部”默认隐藏已完成、列表与甘特同源、整行回捞及恢复默认均通过只读验收。去敏证据见[上线证据](上线证据-生活助手-LifeConsole-2.7.0.md)。
