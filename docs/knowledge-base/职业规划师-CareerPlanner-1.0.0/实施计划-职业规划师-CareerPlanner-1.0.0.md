...
- [ ] **Step 1:** 在结构测试中声明 `career/` 必须被 Git 排除，并运行测试确认失败。
- [ ] **Step 2:** 将 `career/` 加入 Git 排除规则，同时在根仓库本地 exclude 中立即生效。
- [ ] **Step 3:** 用空白 Markdown 文件建立目录，内容只含填写说明、字段和状态，不写虚构经历。
- [ ] **Step 4:** 验证所有入口可读、`git check-ignore career/README.md` 成功且根工作区不显示 `career/`。
...
- [ ] **Step 1:** 先写触发、反触发、原文保护和一主两辅路径的失败测试并确认 RED。
- [ ] **Step 2:** 使用官方 `init_skill.py` 初始化 `plan-career`。
- [ ] **Step 3:** 写最小 SKILL 和五份单层 reference，使测试转绿。
- [ ] **Step 4:** 运行 `quick_validate.py`、聚焦单元测试和项目治理检查。
...
- [ ] **Step 1:** 更新目标和计划，明确睡眠护栏、周末自由与可降级动作。
- [ ] **Step 2:** 写提醒规范 Prompt 和注册表契约，不含私人材料正文。
- [ ] **Step 3:** 创建五个一次性 heartbeat，逐个读回时间、目标线程和 Prompt。
- [ ] **Step 4:** 验证工作日覆盖、周末为零、到期后无继续触发。
...
- [ ] **Step 1:** 运行聚焦测试、全量 tools 测试、项目校验、隐私与差异检查。
- [ ] **Step 2:** 只把真实通过项写入验收和 PMO；真实材料导入保持未授权。
- [ ] **Step 3:** 提交通用文件；私人 `career/`、目标、计划和提醒规格不进入 Git。
- [ ] **Step 4:** 打开 `career/` 给用户检查，并报告提醒状态与未完成项。
