...
- [x] **Step 1:** 建立 `career/个人知识库/`、管理层和四个业务一级类目；旧原始资料目录改为兼容说明。
- [x] **Step 2:** 递归枚举并耗尽两个项目 Wiki 队列；220 个节点保留父子关系、顺序、来源标识和捕获时间。
- [x] **Step 3:** 保存可获得的原生导出、202 份 Docx 离线正文和媒体对象；DLP、HTTP 403 与格式限制逐项标记，未降级为完整。
- [x] **Step 4:** 保存绩效总结原文；本期未进行职业分析、摘要改写或证据提炼。
- [x] **Step 5:** 生成迁移清单、大小、SHA-256、完整性报告、对象清单和恢复说明；39/39 个离线抽样通过，`career/` 未进入 Git 状态或历史。
