...
1. 点击系统页分区 2「下载恢复包」。
2. 弹出对话框：
   - 请输入恢复包保护口令（至少 16 位；允许受信任密码管理器生成和填充）
   - 请再次输入保护口令
...
PO 于 2026-08-11 完成以下设计开放项决策，并明确要求“完成评审，开始代码实现”。设计方案 Gate 2 已通过。

| 编号 | PO 决策 | 落地边界 |
|---|---|---|
| O1 | **迁移向导拆为独立子页 `/system/migration`** | 系统页仅保留迁移阶段、回滚窗口、最近校验结果和入口；计划、校验、切源与回滚流程全部进入独立子页。 |
| O2 | **记录列表显示删除摘要，详情页显示完整倒计时与操作** | 列表仅显示 `D-天数`、警示色和计划状态；取消删除、立即删除、精确剩余时间与影响说明仅在详情页提供。 |
| O3 | **恢复包口令至少 16 位** | 允许长密码短语，不强制大小写/数字/符号组合；提供强度提示与常见弱口令拦截。结合 Q1，允许受信任密码管理器生成和填充。 |
| O4 | **2.0.0 首版不提供审计 CSV 导出** | 首版仅提供站内筛选、分页和详情；后续确有需求时另行评审脱敏字段、下载授权和导出审计。 |
...
- 确认日期：2026-08-11。
- 修改意见：无；按 O1-O4 决策实施。
- 授权边界：允许开始通用代码与合成测试实现；真实密钥、正式部署、真实数据迁移和切源仍按阶段 C-E 单独确认。
