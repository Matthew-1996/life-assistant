...
日记由对话触发和整理，用户不需要在文档里重复填表。自 Life Console 2.3.0 切源后，Supabase 是活跃日记的唯一真相源；本目录保存切源前历史快照、恢复资料与日记治理说明，不再接受活跃新增。

## 当前统一写入与整理

- 普通新增按 [`QUICK_CAPTURE.md`](QUICK_CAPTURE.md) 调用 `tools/life_console_cloud.py`，先保存原文，再写整理结果；任何失败都不得回退生成第二份本地活跃记录。
- Agent、Life Console 表单/对话和未来自动入口共同使用唯一版本化契约 [`journal-normalization-v1.json`](../apps/life-console/contracts/journal-normalization-v1.json)。契约正文是字段、证据规则和整理 Prompt 的唯一维护位置。
- Agent 可直接提交符合契约的整理结果；不能触发 Agent 的入口才允许走经单独上线授权的服务端 DeepSeek 兜底。供应商失败只改变整理状态，不影响已保存原文。
- 当前不批量重写历史日记；旧记录只有在未来明确授权时才按同一契约重整。
...
- 活跃云端记录固定为 Owner 私有访问；iCloud 只保存受控备份与切源前历史快照，不是第二个活跃真相源。旧快照中的 `local-only` 是历史数据标签，不代表当前新增仍写本地。完整数据流、备份和删除边界见 [生活日记隐私与删除边界](PRIVACY.md)。
...
以下本地工具只用于历史读取、更正、恢复与完整性维护；在线主源标记存在时，`journal_manager.py add` 会拒绝活跃新增。当前新增入口见上方“当前统一写入与整理”。
