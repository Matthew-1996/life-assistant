...
规范正文是 PO 原文的逐字副本，不在 Git 流程中直接改写。若 PO 更新规范，必须从 PO 维护的原始文件逐字同步，运行 `python3 tools/check_project_governance.py` 校验正文完整性，并在 PR 中列明正文哈希变化与 PO 确认；Agent 补充解释应放在本文件、`AGENTS.md` 或知识库中。
