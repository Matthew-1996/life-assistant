...
`docs/governance/agent-user-project-development-standard.md` 是本地项目与 GitHub 的唯一规范正文，不维护另一份“原始文件”。若 PO 更新规范，直接修改该文件，运行 `python3 tools/check_project_governance.py` 校验正文完整性，并在 PR 中列明正文哈希变化与 PO 确认；Agent 补充解释应放在本文件、`AGENTS.md` 或知识库中。旧路径如需兼容，只能指向该文件，不得保存第二份正文。
