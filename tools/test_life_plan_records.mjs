...
try {
  ...
  const scaffold = weeklyScaffoldRows();
  ...
  assert.equal(scaffold[4].phase, "阶段 2 → 阶段 3");
  assert.equal(scaffold[6].phase, "阶段 3 → 阶段 4");
  assert.equal(scaffold[10].phase, "阶段 4 → 阶段 5");
  assert.equal(scaffold.at(-1).iso_week, "2026-W43");
  ...
  assert.equal(weeklyPlan.length, 12);
  assert.equal(weeklyPlan[0].values.length, 10);
  assert.equal(weeklyPlan[0].values[2], "2026-W32");
  ...
  await fs.writeFile(weeklyPath, `${JSON.stringify(weeklyReview({ week_start: "2026-08-04" }))}\n`);
