...
class LifeAssistantStatusTest(unittest.TestCase):
    ...
    def test_portable_contract_covers_daily_upsert_and_natural_week(self) -> None:
        ...
        self.assertIn("tools/daily_checkin.py upsert", prompt)
        self.assertIn("tools/daily_checkin.py purge-plan", prompt)
        self.assertIn("record_etag", prompt)
        self.assertIn("tools/google_sheets_payload.mjs", prompt)
        self.assertIn("tools/google_sheets_state.py mark-success", prompt)
        self.assertIn("每日记录”D:P、每周复盘和日记索引都是只读派生视图", prompt)
        self.assertIn("外部失败不得回滚本地写入", prompt)
        self.assertIn("即使是周日也按普通日期", prompt)
        self.assertIn("除 2026-08-02 外，如果当天是周日", prompt)
        self.assertIn("除 2026-08-02 外，如果当天是周一", prompt)
        self.assertIn("journal_manager.py review-plan --type weekly", prompt)
        self.assertIn("跳过重复询问", prompt)
        self.assertIn("这四个分数都记在 YYYY-MM-DD", prompt)
        self.assertIn("差值 `≤60` 分钟采用设备精确值", prompt)
        self.assertIn("截至回访时的 --energy、--mood、--life-feeling", prompt)
        self.assertIn("昨天只写入回顾的 --life-action 和 --wind-down", prompt)
        self.assertIn("--clear-field 与当前 --expect-revision", prompt)
        self.assertIn("静默日记维护仍须执行", prompt)
        self.assertIn("周一至周日自然周", prompt)
        self.assertIn("有帮助、一般还是打扰", prompt)
        self.assertIn("继续每日、降为每周、仅按需、暂停或暂不决定", prompt)
        self.assertIn("第一条阶段复盘最多三个回答组", prompt)
        self.assertIn("不在同一条消息追问身体限制", prompt)
        self.assertIn("先展示一次性提醒的日期、当地时间和目标", prompt)
        self.assertIn("取得当次明确确认后才创建提醒", prompt)
        self.assertIn("2026-08-14 之后没有新选择就保持安静", prompt)
        self.assertIn("仅回复“今天跳过”", prompt)
        self.assertIn("即使正文偶然提到睡眠或情绪", prompt)
        self.assertIn("普通问题、提醒时间或频率设置", prompt)
        self.assertIn("action 为 created、updated 或 unchanged", prompt)
