...
function timestamp() {
  return "2026-01-07T03:00:00Z";
}

try {
  await fs.mkdir(path.join(root, "journal"), { recursive: true });
  await fs.mkdir(path.join(root, "records"), { recursive: true });
  // 合成阶段路线：与真实个人数据无关；窗口与真实数据等长（92 天、12 周），
  // 使 clear_ranges / format_updates 的长度派生输出保持一致。
  await fs.writeFile(
    path.join(root, "life-plan-schedule.json"),
    JSON.stringify({
      anchor_week_start: "2026-01-05",
      weeks: 12,
      phases: [
        { id: "1", start: "2026-01-05", end: "2026-01-18", title: "阶段甲" },
        { id: "2", start: "2026-01-19", end: "2026-02-08", title: "阶段乙" },
        { id: "3", start: "2026-02-09", end: "2026-03-01", title: "阶段丙" },
        { id: "4", start: "2026-03-02", end: "2026-03-22", title: "阶段丁" },
        { id: "5", start: "2026-03-23", end: "2026-04-06", title: "阶段戊" },
      ],
    }),
  );
  const journal = {
