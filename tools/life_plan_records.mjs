...
const phaseSchedule = [
  ["2026-08-01", "2026-08-14", "阶段 1｜稳住睡眠与生活感", "阶段 1"],
  ["2026-08-15", "2026-08-31", "阶段 2｜低耗完成交接", "阶段 2"],
  ["2026-09-01", "2026-09-14", "阶段 3｜离职后减压", "阶段 3"],
  ["2026-09-15", "2026-10-12", "阶段 4｜重建生活结构", "阶段 4"],
  ["2026-10-13", "2026-10-31", "阶段 5｜选择下一重点", "阶段 5"],
];

function phaseForDate(value) {
  const phase = phaseSchedule.find(([start, end]) => start <= value && value <= end);
  if (!phase) throw new Error(`自然周日期 ${value} 不在当前阶段路线内`);
  return phase;
}

function phaseForWeek(weekStart, weekEnd) {
  const startPhase = phaseForDate(weekStart);
  const endPhase = phaseForDate(weekEnd);
  return startPhase[3] === endPhase[3]
    ? startPhase[2]
    : `${startPhase[3]} → ${endPhase[3]}`;
}

export function weeklyScaffoldRows() {
  return Array.from({ length: 12 }, (_, index) => {
    const weekStart = addIsoDays("2026-08-03", index * 7);
    const weekEnd = addIsoDays(weekStart, 6);
    return {
      week_start: weekStart,
      week_end: weekEnd,
      iso_week: isoWeekKey(weekStart),
      phase: phaseForWeek(weekStart, weekEnd),
    };
  });
}
